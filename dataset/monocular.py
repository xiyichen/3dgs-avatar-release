...
class MonocularDataset(Dataset):
    ...
    def getitem(self, idx, data_dict=None):
        ...
        # joints_cam = world_to_cam(Jtr_posed, torch.tensor(R.T).float(), torch.tensor(T).float())
        # joints_cam = joints_cam/joints_cam[:, -1:]
        # joints_projected = torch.einsum('ij,kj->ki', torch.from_numpy(K).float(), joints_cam)[:, :2]
        # print(joints_projected)
        # im = im*255
        
        # for idx_, loc in enumerate(joints_projected):
        #     x = int(loc[0])
        #     y = int(loc[1])
        #     cv2.circle(im, (x, y), 1, (255, 0, 0), -1)
        # cv2.imwrite('./smpl_debug.png', im[:,:,::-1])
        # exit()
